...
# Interface + gateway endpoints so private tasks reach AWS APIs without the NAT bill.
...
resource "aws_vpc_endpoint" "interface" {
  for_each            = toset(["ecr.api", "ecr.dkr", "secretsmanager", "logs", "ssm", "bedrock-runtime"])
  vpc_id              = aws_vpc.this.id
  service_name        = "com.amazonaws.${var.region}.${each.key}"
