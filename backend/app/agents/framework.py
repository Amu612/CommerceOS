...
class DomainAgent:
    ...
    def query(self, message: str, db=None, **_: Any) -> AgentQueryResponse:
        return self.chat(message, db=db)
    ...
    # ── chat ───────────────────────────────────────────────────
    def chat(self, message: str, db=None) -> AgentQueryResponse:  # noqa: ARG002
        ...
        if model is not None:
            try:
                answer = self._react_chat(model, message)
                if answer:
        ...
        return AgentQueryResponse(
            ...
            answer=self._deterministic_chat(message, analysis), data=context, llm_backed=False,
        )

    def _react_chat(self, model, message: str) -> str:
        ...
        agent = create_react_agent(
            ...
            prompt=(
                ...
                "e-commerce database. Call the tools you need, then answer the user's question "
                "concisely and quantitatively. Never invent numbers, entities, or categories — "
                "only use tool output. If the tools don't cover the question, say so."
            ),
        )
        result = agent.invoke(
    ...
    def _deterministic_chat(self, message: str, analysis: AgentAnalysisOutput) -> str:
        """
        No-LLM path: still runs the LangChain tools. Picks the tool(s) whose name
        or description best matches the question, executes them, and renders their
        real output. Falls back to the analysis findings/metrics.
        """
        ...
        for _, t in scored[:2]:
            try:
                ...
                lines.append(f"**{t.name.replace('_', ' ').title()}**:")
        ...
        lines += ["", "*Deterministic (no LLM key). Add `GROQ_API_KEY=` to `backend/.env` and restart for conversational answers.*"]
        return "\n".join(lines)
    ...
    if isinstance(out, dict):
        ...
        for v in out.values():
            if isinstance(v, list) and v and isinstance(v[0], dict):
                return [f"- " + ", ".join(f"{k}: {vv}" for k, vv in row.items()) for row in v[:8]]
        return [f"- {k}: {v}" for k, v in out.items() if not isinstance(v, (dict, list))][:12]
    if isinstance(out, list):
        return [f"- " + ", ".join(f"{k}: {vv}" for k, vv in row.items()) if isinstance(row, dict) else f"- {row}"
                for row in out[:8]]
    return [f"- {out}"]
