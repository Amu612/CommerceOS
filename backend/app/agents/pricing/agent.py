        ...
        if tool_name == "margin_overview" and data.get("status") == "OK":
            ...
            cards = [
                MetricCard(
                    ...
                    value=f"{data['blended_margin_pct']}%",
                    description=f"{money(data['total_profit'])} on {money(data['total_revenue'])}",
                    data_status="CALCULATED",
                ),
                MetricCard(
                    label="Median Order Margin",
                    value=f"{data['median_order_margin_pct']}%",
                    description=f"P25 {data['p25_margin_pct']}%",
                    data_status="CALCULATED",
                ),
                MetricCard(
                    label="Loss-Making Orders",
                    value=f"{data['loss_making_rate_pct']}%",
                    description=f"{data['loss_making_orders']:,} orders below zero margin",
                    data_status="CALCULATED",
                ),
            ]
            ...
            chart = segs
        elif tool_name == "category_price_benchmarks" or (
            tool_name == "discount_leakage" and data.get("available")
        ):
