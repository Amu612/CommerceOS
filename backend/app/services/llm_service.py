...
class _LLMServiceShim:
    ...
    def generate_text(self, system_prompt: str, user_message: str, max_tokens: int = 1000) -> str | None:
        text = get_llm().generate_text(
            system=system_prompt, user=user_message, max_tokens=max_tokens, untrusted=True
        )
        return text or None

    def generate_json(self, system_prompt: str, user_message: str) -> dict[str, Any] | None:
