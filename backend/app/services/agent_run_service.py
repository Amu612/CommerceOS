...
def record_analysis(output: Any, *, agent: str, trigger: str = "api", latency_ms: float | None = None,
                    propose_actions: bool = True) -> str | None:
    ...
    try:
        ...
        run = AgentRun(
            ...
            health=getattr(output, "health", None) or getattr(getattr(output, "health", None), "status", None),
        ...
        for f in findings:
            db.add(AgentFinding(
                id=_uuid(), run_id=run.id, agent=agent,
        ...
        db.commit()
        publish_event("agent_runs", {
            "type": "run_completed", "agent": agent, "run_id": run.id,
            "health": run.health, "findings": len(findings), "llm_backed": run.llm_backed,
        })
