...
class ReplayEngine:
    ...
    def index_events_from_datasets(self, max_orders: int = 5000) -> int:
        ...
        try:
                    ...
                    for _, row in df_cust.iterrows():
                        ...
                        if cid in target_customer_ids or len(self._customer_cache) < 20000:
                            self._customer_cache[cid] = {
                                ...
                                "customer_zip_code_prefix": int(row["customer_zip_code_prefix"]) if pd.notna(row.get("customer_zip_code_prefix")) else 1000,
                    ...
                    for _, row in df_prod.iterrows():
                        ...
                        if pid in target_product_ids or len(self._product_cache) < 20000:
                            self._product_cache[pid] = {
                                "product_id": pid,
                                "product_category_name": str(row.get("product_category_name")) if pd.notna(row.get("product_category_name")) else "general",
                    ...
                    for _, row in df_sell.iterrows():
                        ...
                        if sid in target_seller_ids or len(self._seller_cache) < 10000:
                            self._seller_cache[sid] = {
                                "seller_id": sid,
                                "seller_zip_code_prefix": int(row["seller_zip_code_prefix"]) if pd.notna(row.get("seller_zip_code_prefix")) else 0,
                ...
                try:
                    ...
                    for _, row in df_pay.iterrows():
                        ...
                        olist_payments_by_order[oid].append({
                            "order_id": oid,
                            "payment_sequential": int(row["payment_sequential"]) if pd.notna(row.get("payment_sequential")) else 1,
                ...
                try:
                    ...
                    for _, row in df_rev.iterrows():
                        ...
                        olist_reviews_by_order[oid].append({
                            ...
                            "review_creation_date": row["review_creation_date"].to_pydatetime() if pd.notna(row.get("review_creation_date")) else None,
                            "review_answer_timestamp": row["review_answer_timestamp"].to_pydatetime() if pd.notna(row.get("review_answer_timestamp")) else None,
                        })
            ...
            for row in olist_order_rows:
                ...
                payment_total = sum(p["payment_value"] for p in payments) if payments else (total_price + total_freight)

                # Real customer metadata
                cust_meta = self._customer_cache.get(cid, {
                    "customer_id": cid,
                ...
                raw_events.append({
                    ...
                    "approved_at": row["order_approved_at"].to_pydatetime() if pd.notna(row.get("order_approved_at")) else None,
                    "delivered_carrier_date": row["order_delivered_carrier_date"].to_pydatetime() if pd.notna(row.get("order_delivered_carrier_date")) else None,
            ...
            if dc_path:
                try:
                    ...
                    df_dc_items = pd.read_csv(dc_path, usecols=dc_item_cols, nrows=max_orders * 3, encoding="latin1")
    ...
    def _index_from_database(self, max_orders: int = 5000) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for o in orders:
                purch_ts = o.order_purchase_timestamp or datetime.now(timezone.utc)
                if purch_ts.tzinfo is None:
                    purch_ts = purch_ts.replace(tzinfo=timezone.utc)
                ...
                events.append({
                    ...
                    "estimated_delivery_date": o.order_estimated_delivery_date or purch_ts + timedelta(days=7),
                    "is_delayed": is_delayed,
                    "merchandise_revenue": total_price,
                    "freight_value": total_freight,
                    "payment_total": payment_total,
            ...
            if dc_oids:
                for i in range(0, len(dc_oids), 500):
                    ...
                    for dci in db.query(DataCoOrderItem).filter(DataCoOrderItem.order_id.in_(chunk_dco)).all():
            ...
            for dco in dc_orders:
                order_ts = dco.order_date or datetime.now(timezone.utc)
                if order_ts.tzinfo is None:
                    order_ts = order_ts.replace(tzinfo=timezone.utc)

                dc_items = dc_items_by_order.get(dco.order_id, [])
                item_count = len(dc_items) if dc_items else 1
                total = sum(it["order_item_total"] for it in dc_items) if dc_items else float(dco.order_total or 50.0)
                profit = sum(it["order_profit_per_order"] for it in dc_items) if dc_items else float(dco.order_profit or 5.0)
    ...
    def _generate_synthetic_stream(self, count: int = 100) -> List[Dict[str, Any]]:
        ...
        events = []
        cities = [("Sao Paulo", "SP"), ("Rio de Janeiro", "RJ"), ("Belo Horizonte", "MG"), ("Curitiba", "PR"), ("Salvador", "BA")]
        categories = ["bed_bath_table", "health_beauty", "sports_leisure", "computers_accessories", "furniture_decor"]
    ...
    async def start(self) -> str:
        ...
        if self._current_index >= len(self._events_index) and len(self._events_index) > 0:
            return "All indexed events have already been ingested. Reset ingestion to replay from the beginning."

        if self.status == "running" and self._replay_task and not self._replay_task.done() and not self._replay_task.cancelled():
    ...
    def complete_now(self) -> str:
        ...
        if caller and ("main.py" in caller.f_code.co_filename or caller.f_code.co_name == "lifespan"):
            logger.info("Startup complete_now skipped: data ingestion operates strictly through button controls.")
        ...
        for chunk_start in range(self._current_index, end_index, CHUNK_SIZE):
            ...
            try:
                    ...
                    # 3. Prepare DataCo DB records
                        ...
                        for order_item in item.get("order_items", []):
                            dataco_items_records.append({
                                ...
                                "order_item_discount_rate": order_item.get("order_item_discount_rate", 0.0),
                ...
                if olist_records:
                    ...
                    existing_cids = set(
                        r[0] for r in db.query(Customer.customer_id).filter(Customer.customer_id.in_(cids_in_chunk)).all()
                    )
                    new_custs = []
                ...
                if olist_items_records:
                    ...
                    existing_prods = set(r[0] for r in db.query(Product.product_id).filter(Product.product_id.in_(pids_in_chunk)).all())
                    existing_sellers = set(r[0] for r in db.query(Seller.seller_id).filter(Seller.seller_id.in_(sids_in_chunk)).all())
                ...
                if dataco_records:
                    ...
                    existing_dc_ids = set(
                        r[0] for r in db.query(DataCoOrder.order_id).filter(DataCoOrder.order_id.in_(dc_ids)).all()
                    )
                ...
                if dataco_items_records:
                    ...
                    existing_dc_items = set(
                        r[0] for r in db.query(DataCoOrderItem.order_item_id).filter(DataCoOrderItem.order_item_id.in_(item_ids)).all()
                    )
