...
def create(body: CreateUserRequest, admin: User = Depends(require_super_admin), db: Session = Depends(get_db)):
    if get_user_by_username(db, body.username):
        raise ConflictException("Username already exists.")
    user = create_user(db, username=body.username, email=body.email, password=body.password, role=body.role)
    log_audit(db, action="USER_CREATE", actor_id=admin.id, actor_username=admin.username,
              target_type="user", target_id=user.id, details={"role": body.role.value})
    return _public(user)
...
def deactivate(user_id: str, admin: User = Depends(require_super_admin), db: Session = Depends(get_db)):
    ...
    db.commit()
    log_audit(db, action="USER_DEACTIVATE", actor_id=admin.id, actor_username=admin.username,
